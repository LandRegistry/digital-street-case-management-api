...
from case_management_api.models import X500Name
import copy

# Test data
standard_str1 = "CN=Generic Conveyancing Company,OU=Digital,O=Generic Conveyancing Company, L=Plymouth, ST=Devon, C=GB"
standard_str2 = "O=Generic Conveyancing Company,L=Plymouth,C=GB,ST=Devon,OU=Digital,CN=Generic Conveyancing Company"
standard_str3 = "O=Generic Conveyancing Company, L=Plymouth, ST=Devon, C=GB"
standard_str4 = "O=Generic Conveyancing Company, L=Plymouth, C=GB"
standard_str5 = "O=Generic Conveyancing Company, L=Plymouth, ST=Devon"
standard_str6 = "O=C, L=P, C=USA"
standard_dict1 = {
...
class TestModelX500Name(TestCase):
    ...
    def test_003_x500name_from_string(self):
        ...
        x500name = X500Name.from_string(standard_str1)
        self.compare_against_standard(x500name, True, True, True)

        x500name = X500Name.from_string(standard_str2)
        self.compare_against_standard(x500name, True, True, True)

        x500name = X500Name.from_string(standard_str3)
        self.compare_against_standard(x500name, True, False, False)

        x500name = X500Name.from_string(standard_str4)
        self.compare_against_standard(x500name, False, False, False)

    def test_004_x500name_from_string_missing_val(self):
        """Required values missing from serialised string."""
        with self.assertRaises(TypeError) as e:
            X500Name.from_string(standard_str5)
    ...
    def test_005_x500name_from_string_fail_validate(self):
        ...
        with self.assertRaises(ValueError) as e:
            X500Name.from_string(standard_str6)
    ...
    def test_015_x500name_str(self):
        ...
        x500name = X500Name.from_dict(standard_dict1)
        self.assertEqual(str(x500name), standard_str2)

    def test_016_x500name_repr(self):
        """Displays the X500Name as a serialised string."""
        x500name = X500Name.from_dict(standard_dict1)
        self.assertEqual(repr(x500name), standard_str2)
